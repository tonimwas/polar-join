...
function App() {
  ...
  useEffect(() => {
    if (form.type === 'polar' && form.polarEa && form.polarNa) {
      ...
      const newEndE = (parseFloat(form.polarEa) + (delta_e || 0)).toFixed(3);
      const newEndN = (parseFloat(form.polarNa) + (delta_n || 0)).toFixed(3);
      setEndpointCoords({ e: newEndE, n: newEndN });
      // Check if the new coordinates match a saved point
      const match = savedPoints.find(pt => 
        Number(pt.e).toFixed(3) === newEndE && 
        Number(pt.n).toFixed(3) === newEndN
      );
      if (match) {
        if (form.polarEndName !== match.name || !savedStatus.polarEnd) {
  ...
  return (
    <div className="calculator-container">
      {/* Compact Saved Points Table (moved above calculator) */}
        ...
        <div className="precision-row">
            ...
            <input
              ...
              onChange={e => {
                ...
                if (val < 0) {
                  setError('Decimal places cannot be negative');
                } else {
                  setError(null);
                  setPrecision(val);
                }
              ...
              <div className="form-group join-pair" style={{marginTop: '16px'}}>
                ...
                <div>
                  ...
                  <input
                    ...
                    id="polarEndE"
                    name="polarEndE"
                    value={(parseFloat(form.polarEa) + (result.delta_e || 0)).toFixed(3)}
                    readOnly
                    step="any"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    id="polarEndN"
                    name="polarEndN"
                    value={(parseFloat(form.polarNa) + (result.delta_n || 0)).toFixed(3)}
                    readOnly
                    step="any"
                  />
                </div>
            ...
            <tbody>
              {savedPoints.map((pt, idx) => (
                <tr key={idx}>
                  ...
                  <td className="saved-points-cell">{pt.e}</td>
                  <td className="saved-points-cell">{pt.n}</td>
      ...
      {result && (
        <div className="result-box">
          ...
          {/* Endpoint naming and saving UI for polar mode */}
          {form.type === 'polar' && form.polarEa && form.polarNa && result && (
            <div className="form-group join-pair" style={{marginTop: '16px'}}>
              <div className="points-flex-container">
                <div>
                  ...
                  <input
                    ...
                    name="polarEndName"
                    value={form.polarEndName || ''}
                    onChange={e => setForm(f => ({ ...f, polarEndName: e.target.value }))}
                    placeholder="Name for endpoint"
                    className="point-name-input"
                  />
                </div>
                <span
                  ...
                  onClick={() => handleSavePoint(
                    form.polarEndName,
                    (parseFloat(form.polarEa) + (result.delta_e || 0)).toFixed(3),
                    (parseFloat(form.polarNa) + (result.delta_n || 0)).toFixed(3),
                    'polarEnd')}
                >
                  {savedStatus.polarEnd ? '✔' : '📋'}
                </span>
